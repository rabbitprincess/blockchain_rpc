...
const (
	...
	// - Not forwarded
	// - No fee check
	telLOCAL_ERROR TxResult = iota - 399
	telBAD_DOMAIN
	telBAD_PATH_COUNT
	telBAD_PUBLIC_KEY
	...
	temMALFORMED TxResult = iota - 299
	temBAD_AMOUNT
	temBAD_CURRENCY
	temBAD_EXPIRATION
	temBAD_FEE
...
var resultNames = map[TxResult]struct {
	Token string
	Human string
}{
...
var reverseResults map[string]TxResult

func init() {
	reverseResults = make(map[string]TxResult)
...
func (r TxResult) Queued() bool {
	return r == terQUEUED
}

func (r TxResult) Symbol() string {
	switch r {
